...
void Map::UpdateForPlayersInNewSquare(Unit* p_Unit, bool p_UpdateAll)
{
    ...
    for (uint16 l_Id : l_DiffSquareSet)
    {
        ...
        for (std::pair<uint16, Unit*> l_UnitPair : *l_SquareList)
        {
            ...
            l_Session->SendUnitCreate(p_Unit->GetType(), p_Unit->GetID(), p_Unit->GetName(), p_Unit->GetLevel(), p_Unit->GetHealth(), p_Unit->GetSkinID(), p_Unit->GetMapID(), p_Unit->GetPosX(), p_Unit->GetPosY(), p_Unit->GetOrientation(), p_Unit->IsInMovement(), p_Unit->GetMovementHandler()->IsInAttack());
        }
    }
}
